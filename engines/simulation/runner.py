...
from engines.analysis.news_factors import analyze_news_for_match, enrich_factors
from engines.data.sources.odds import OddsClient
from db.database import SessionLocal
from db.models import Match, SimulationResult, Team

def compute_real_factors(match) -> tuple:
    db = SessionLocal()
    ...
    db.close()
    return inputs, odds, home.name, away.name
...
def run_simulation(match_id: int, profile_id: int = None, n: int = 10000) -> dict:
    ...
    factor_inputs, odds, home_name, away_name = compute_real_factors(match)

    # LLM news analysis
    news_data = analyze_news_for_match(home_name, away_name)
    factor_inputs = enrich_factors(factor_inputs, news_data, home_name, away_name)
    factor_inputs['_news_source'] = news_data.get('source', 'none')
    ...
    engine.close()
    result["odds"] = odds
    result["news_source"] = factor_inputs.get("_news_source", "none")
    return result